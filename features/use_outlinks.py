...
def classify_link_quality(link: Dict, source_domain: str) -> str:
    url = link['url']
    ...
    quality_domains = {
        ...
        'w3.org', 'ieee.org', 'acm.org', 'arxiv.org', 'pubmed.ncbi.nlm.nih.gov',
        'scholar.google.com', 'nature.com', 'science.org', 'bbc.com', 'reuters.com',
        'ap.org', 'npr.org', 'pbs.org'
    }
    ...
    if any(domain in link_domain for domain in quality_domains):
        return 'quality'
...
def analyze_link_patterns(links: List[Dict], source_domain: str) -> Dict:
    if not links:
        return {
            'total_outlinks': 0,
            'internal_links': 0,
    ...
    for link in links:
        try:
            ...
            classification = classify_link_quality(link, source_domain)
...
def analyze_outlinks(url: str) -> Dict:
    ...
    link_stats = analyze_link_patterns(links, source_domain)
